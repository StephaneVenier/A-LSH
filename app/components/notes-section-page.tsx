...
import { sectionInfo, type NoteSection } from "@/lib/notes/sections";
...
export default async function NotesSectionPage({
  searchParams,
  section,
}: {
  section: NoteSection;
  ...
  const message = readParam("moved") ? "Note déplacée." : readParam("created") ? "Note créée." : readParam("updated") ? "Note modifiée." : readParam("deleted") ? "Note supprimée définitivement." : "";

  return (
    <section className="animate-[fade-in_500ms_ease-out]">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--coral-dark)]">A-LSH · Notes</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-[var(--ink)] sm:text-5xl">{info.label}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--muted)] sm:text-base">Vos notes personnelles, au même endroit. Elles restent strictement privées.</p></div>{section === "notebook" ? <Link href="/cahier/nouvelle" className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-[var(--coral)] px-5 text-sm font-bold text-white shadow-[0_12px_24px_rgba(228,121,95,0.22)] hover:bg-[var(--coral-dark)]">+ Nouvelle note</Link> : null}</div>
      <div className="mt-8"><NoteFilters section={section} search={search} category={category} pinned={pinned} /></div>
