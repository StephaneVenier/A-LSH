...
import { EventPreview } from "@/app/components/event-preview";
...
export default async function DashboardPage() {
  ...
  const { data: eventData, error: eventError } = await supabase.from("events").select("id, workspace_id, created_by, updated_by, title, description, location, category, color, start_date, start_time, end_date, end_time, is_all_day, visibility, created_at, updated_at").eq("workspace_id", context.workspace.id).lte("start_date", today).gte("end_date", today).order("start_time", { ascending: true, nullsFirst: true }).limit(6);
  const dashboardTasks = (taskData ?? []) as TaskRecord[];
  const dashboardEvents = (eventData ?? []) as EventRecord[];
    ...
    <section className="animate-[fade-in_500ms_ease-out]">
      <div className="rounded-[2rem] border border-[var(--line)] bg-white/90 p-7 shadow-[0_24px_80px_rgba(23,48,43,0.1)] sm:p-12">
            ...
            <div className="mt-12 border-t border-[var(--line)] pt-8">
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-2"><div className="rounded-3xl border border-[var(--line)] bg-white p-5"><div className="flex items-center justify-between gap-3"><h3 className="font-bold text-[var(--ink)]">Tâches à suivre</h3><Link href="/taches" className="text-xs font-bold text-[var(--green)] hover:underline">Toutes</Link></div>{taskError ? <p className="mt-4 text-sm text-[var(--ink-soft)]">Les tâches ne sont pas disponibles pour le moment.</p> : overdueTasks.length || todayTasks.length ? <div className="mt-4 space-y-4">{overdueTasks.length ? <div><p className="mb-2 text-xs font-bold uppercase tracking-[0.14em] text-[var(--coral-dark)]">En retard</p><div className="space-y-2">{overdueTasks.slice(0, 3).map((task) => <TaskPreview key={task.id} task={task} overdue />)}</div></div> : null}{todayTasks.length ? <div><p className="mb-2 text-xs font-bold uppercase tracking-[0.14em] text-[var(--muted)]">Dues aujourd’hui</p><div className="space-y-2">{todayTasks.slice(0, Math.max(0, 4 - Math.min(overdueTasks.length, 3))).map((task) => <TaskPreview key={task.id} task={task} />)}</div></div> : null}</div> : <p className="mt-4 text-sm text-[var(--ink-soft)]">Aucune tâche due aujourd’hui ou en retard.</p>}</div><div className="rounded-3xl border border-[var(--line)] bg-white p-5"><div className="flex items-center justify-between gap-3"><h3 className="font-bold text-[var(--ink)]">Événements du jour</h3><Link href="/agenda" className="text-xs font-bold text-[var(--green)] hover:underline">Agenda</Link></div>{eventError ? <p className="mt-4 text-sm text-[var(--ink-soft)]">Les événements ne sont pas disponibles pour le moment.</p> : dashboardEvents.length ? <div className="mt-4 space-y-2">{dashboardEvents.slice(0, 4).map((event) => <EventPreview key={event.id} event={event} />)}</div> : <p className="mt-4 text-sm text-[var(--ink-soft)]">Aucun événement prévu aujourd’hui.</p>}</div></div>
